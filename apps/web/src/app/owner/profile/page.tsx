...
import { Phone, Mail, ShieldCheck, ShieldAlert, Percent, LogOut } from 'lucide-react';
      ...
      <Card className="border-0 shadow-sm">
        <CardContent className="p-5 flex flex-col items-center text-center">
          <div className="w-16 h-16 rounded-full bg-amber-100 flex items-center justify-center text-xl font-bold text-amber-700 mb-3">
            {isLoading ? '' : (p?.fullName?.[0]?.toUpperCase() ?? 'O')}
          </div>
          {isLoading ? (
        ...
        <CardContent className="p-4 flex flex-col divide-y divide-gray-100">
          ...
          {p?.mgmtFeePct != null && (
            <div className="flex items-center gap-3 py-2.5">
              ...
              <span className="text-sm text-gray-700">{p.mgmtFeePct}% management fee</span>
            </div>
          )}
        </CardContent>
      </Card>
